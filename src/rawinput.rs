use winapi::um::winuser::{RAWINPUT, RIM_TYPEMOUSE, GetRawInputDeviceInfoW, RAWINPUTDEVICELIST, RIM_TYPEKEYBOARD, RIDI_PREPARSEDDATA, PRAWINPUT, GetRawInputDeviceList, RIDI_DEVICEINFO, RID_DEVICE_INFO, GetRawInputBuffer, RIM_TYPEHID, RIDI_DEVICENAME, PRAWINPUTDEVICELIST, RAWINPUTHEADER};
...
use winapi::shared::hidpi::{HidP_GetCaps, HIDP_STATUS_SUCCESS, PHIDP_PREPARSED_DATA, PHIDP_VALUE_CAPS, HidP_GetButtonCaps, HIDP_CAPS, HidP_GetValueCaps, HIDP_BUTTON_CAPS, HidP_Input, HIDP_VALUE_CAPS, PHIDP_BUTTON_CAPS};
use winapi::um::fileapi::{OPEN_EXISTING, CreateFileW};
use winapi::um::handleapi::INVALID_HANDLE_VALUE;
use devices::{JoystickInfo, Devices, KeyboardInfo, MouseInfo, JoystickState, DeviceInfo};
use event::RawEvent;
...
fn read_input_buffer(event_queue: &mut VecDeque<RawEvent>, devices: &mut Devices) {
    unsafe{
        let mut array_alloc: [u8;16000] = mem::uninitialized();
        let mut buffer_size: UINT = 0;

        let mut numberofelements: i32 =
            GetRawInputBuffer(
        ...
        numberofelements =
            GetRawInputBuffer(
                array_alloc.as_mut_ptr() as PRAWINPUT, &mut buffer_size,
                mem::size_of::<RAWINPUTHEADER>() as UINT,
                ) as INT;
...
pub unsafe fn raw_name_to_hid(name: String) -> Result<HANDLE, String> {
    ...
    let mut classname = os_name.encode_wide().chain(Some(0).into_iter())
        .collect::<Vec<_>>();
    let hid_handle = CreateFileW(classname.as_mut_ptr(),
                                 0,
...
pub unsafe fn get_device_info(handle: HANDLE, name: String, serial: Option<String>
    ...
    return match raw_info.dwType {
        RIM_TYPEMOUSE =>
            Some(DeviceInfo::Mouse(
                MouseInfo{names: vec!(name),
                          handles: vec![handle],
                          serial: serial,
                          info: raw_info}
                )),
        RIM_TYPEKEYBOARD =>
            Some(DeviceInfo::Keyboard(
                KeyboardInfo{names: vec!(name),
                             handles: vec![handle],
                             serial: serial,
                             info: raw_info}
                )),
        RIM_TYPEHID => {
            ...
            assert!(GetRawInputDeviceInfoW(handle,
                                           RIDI_PREPARSEDDATA,
                                           ptr::null_mut(),
...
pub unsafe fn get_serial_number(handle: HANDLE) -> Option<String> {
    ...
    let result = HidD_GetSerialNumberString(
        handle, string_buffer.as_mut_ptr() as PVOID, string_buffer_size);
    let serial_string_unparsed =
        match OsString::from_wide(&string_buffer[0..128]).into_string() {
        Ok(something) => something,
    ...
    if result == 1{
        ...
        if let Some(string_index) = string_front {
            if string_index > 10 {
                serial_string =
                    Some(String::from(&serial_string_unparsed[0..string_index]));
            }
        }
    }
    serial_string
